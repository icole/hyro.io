    ...
    async placeOffer() {
      ...
      if (contract) {
        if (offerAmount) {
          ...
          this.get('spinner').show('offer-processing');
          ...
          try {
            ...
            offerEvent.watch(async function(error, result) {
              if(self.get('offerResult') && self.get('offerResult.tx') == result.transactionHash) {
                ...
                self.get('spinner').hide('offer-processing');
            ...
            let offerResult = await galleryInstance.placeOffer(web3.get('web3Instance').toWei(offerAmount, "ether"), artPieceId, edition, {
            ...
            console.error(err);
            this.get('spinner').hide('offer-processing');
